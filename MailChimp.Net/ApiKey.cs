...
namespace MailChimp.Net
{
    public class ApiKey
    {
        ...
        public ApiKey()
        {
            // try to load API Key from conig
            var strKey = ConfigurationManager.AppSettings[Config.ConfigKeyName_ApiKey];
            if (string.IsNullOrEmpty(strKey))
                throw new ArgumentNullException("Could not determine API key from config");
